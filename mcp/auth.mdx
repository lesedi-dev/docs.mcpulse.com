...
Read the signed-in account's analytics. That is the whole scope, and it is bounded by the tools that exist rather than by a claim: [six read-only tools](/mcp/tools), no writes.
