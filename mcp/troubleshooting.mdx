...
<AccordionGroup>
  ...
  <Accordion title="The agent cannot write anything">
    Correct, and permanent. The MCP server registers six read-only tools and no others. Nothing there mints a key, renames an MCP or touches the team.

    Use the dashboard or the [REST API](/api/introduction) for writes.
  </Accordion>
</AccordionGroup>
